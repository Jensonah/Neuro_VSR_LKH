...
Node *Best5OptMove(Node *t1, Node *t2, GainType *G0, GainType *Gain)
{
    ...
    // ARE THEY HANDLING ARRAYS OR POINTERS???
    // Candidate set is an array, but handling arrays in this case would seem strange to me, or they are pointers to the specific element in the array? Seems way more logical

    /* Choose (t2,t3) as a candidate edge emanating from t2 */
    Candidate *Ntt2;
    int rest_pts[1000];  // this theoretically could cause out of bound errors
    int number = 0;
    for (Ntt2 = t2->CandidateSet; Ntt2->To; Ntt2++)
    {
        rest_pts[number] = number; // [0,1,2,3,4,5,6,..,max_am_cand,?,..,?]
        number++;
    }
    int point = 0;
    int number2 = number;

    // rest_pts is used as some sort of scratch paper of candidates?

    for (Nt2 = t2->CandidateSet; (t3 = Nt2->To); Nt2++)
    {
        // Here t3 is chosen with the help of 'rest_pts'
        double dtemp = Random() % 100;
        if (dtemp > epsilon) // epsilon is given par (40 is default?)
        {
            point = 0;
            int itemp = rest_pts[point]; // in the first pass (every pass?) still 0
            Ntt2 = t2->CandidateSet + itemp; // get some candidate
            t3 = Ntt2->To; // set t3 as other end of edge as candidate, they also do it like this in the original

            // thus setting t3 as first candidate (following the quality function)
        }
        else
        {
            ...
            // set t3 as random candidate
        }

        // Every entry is shoved one closer to zero
        // rest_pts is a queue!
        if (point < number2 - 1)
        {
        ...
        for (X4 = ProblemType == ATSP ? 2 : 1; X4 <= 2; X4++)
        {
                ...
                (!c || G2 - c(t4, t1) > 0) && (*Gain = G2 - C(t4, t1)) > 0)
            {
                ...
                // ntt2 did not change before
                // update (Q) RL values
                // Note that method 2 only gets updated here, while method 1 always gets updated (see two exact same loops below)
                if (Method == 1 || Method == 2)
                    Ntt2->Value = (1 - Alpha2) * Ntt2->Value + Alpha2 * (R2 + C(t3, t4) - C(t4, t1));
                else
                    Ntt2->Value = *Gain - *G0 + C(t1, t2);

                // But what does the loop do?
                // It iterates through the candidate set

                // Is it queue processing just like with rest_pts?
                // So again iterating over the candidate set?
                for (Candidate *NFrom = t2->CandidateSet; NFrom->To; NFrom++)
                {
                    Candidate Temp = *NFrom;
                    Candidate *NN;
                    // No we are still in the same array? Why do we go backwards?
                    // What does the >= operator do between Candidates
                    // Greater (q) value is better? Yep
                    // So while temp is better than ... 
                    // They are fucking bubble sorting
                    // This code is inefficient, although the list would at most have 10 items... but still!
                    // Couldn't they have put it in a function as well? Man
                    for (NN = NFrom - 1;
                         NN >= t2->CandidateSet &&
                         (Temp.Value > NN->Value ||
                          (Temp.Value == NN->Value && Temp.Alpha < NN->Alpha));
                         NN--)
                         {                         
                        *(NN + 1) = *NN;
                         }
            ...
            if (Method == 1)
            {
                ...
                // Seem to be method_? update lines
            ...
            // Does it start to repeat from here?
            ...
            for (Nt4 = t4->CandidateSet; (t5 = Nt4->To); Nt4++)
            {
                ...
                // Selecting something according to policy or randomly
                dtemp = Random() % 100;
                if (dtemp > epsilon)
                {
                    point4 = 0;
                ...
                if (Method == 2)
                {
                    // Looking for specific component in order to update value
                    ...
                    // Updating value again (why?) 2 should be Q-learning
                    // Method 2 == Q-learning as here it is updated with the maximum - Jens
                    Ntt2->Value = (1 - Alpha2) * Ntt2->Value + Alpha2 * (R2 + Gama * V);
                    // Again sorting here
                    for (Candidate *NFrom = t2->CandidateSet; NFrom->To; NFrom++)
                    {
                ...
                for (X6 = 1; X6 <= 2; X6++)
                {
                        ...
                        (*Gain = G4 - C(t6, t1)) > 0)
                    {
                        ...
                        if (Method == 1 || Method == 2)
                        {
                            // Again one line for updating here
                        ...
                        {
                            // Updating for method 3 here
                            // why t2 here again? Will they later go through 2, 4 and 6? Yes!
                            ...
                            for (Candidate *Nttemp = t4->CandidateSet; Nttemp->To; Nttemp++)
                            {
                                ...
                                // update line (block same structure as above)
